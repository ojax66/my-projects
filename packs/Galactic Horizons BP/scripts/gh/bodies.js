...
const SUN_DISC = [
  ...
  { until: 0.50, block: "gh:sun_blaze" },    // branco, atravessável
  { until: 0.68, block: "gh:sun_flare" },    // amarelo claro
  { until: 0.80, block: "gh:sun_plasma" },   // amarelo
  { until: 0.89, block: "gh:sun_ember" },    // laranja
  { until: 0.95, block: "gh:sun_corona" },   // laranja avermelhado
  { until: Infinity, block: "gh:sun_edge" }, // vermelho
];
...
// Paletas
//
// Blocos próprios (gh:*), com textura feita pra cada corpo — plasma
...
const PALETTES = {
  ...
  earth(x, y, z, body) {
    ...
    if (pole > 0.35) return "gh:earth_ice";
    if (pole > 0 && n > 0.52) return "gh:earth_ice";
    ...
    if (n >= 0.617) return "gh:earth_forest";
    if (n >= 0.536) return "gh:earth_land";
    if (n >= 0.492) return "gh:earth_shallow";
    return "gh:earth_ocean";
  },
  ...
  moon(x, y, z) {
    ...
    if (n >= 0.631) return "gh:moon_regolith_dark";
    if (n >= 0.483) return "gh:moon_regolith";
    return "gh:moon_regolith_light";
  },
  ...
  mars(x, y, z, body) {
    ...
    const pole = polarness(x, y, z, body);
    if (pole > 0.55) return "gh:mars_ice";
    if (pole > 0.2 && n > 0.58) return "gh:mars_ice";

    if (n >= 0.654) return "gh:mars_rock_dark";
