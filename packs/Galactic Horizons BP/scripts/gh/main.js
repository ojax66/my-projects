...
system.afterEvents.scriptEventReceive.subscribe((data) => {
  ...
  // /scriptevent gh:go — atalho pra chegar ao espaço sem subir voando.
  if (data.id === "gh:go") {
  ...
  // /scriptevent gh:tracker — abre o menu sem precisar do item.
  if (data.id === "gh:tracker") {
    if (player?.typeId !== "minecraft:player") return;
    system.run(() => { openTracker(player).catch(() => { }); });
    return;
  }

  // /scriptevent gh:sky — o que está desenhado no céu agora.
  ...
  // /scriptevent gh:info — estado da dimensão e do jogador.
  if (data.id === "gh:info") {
    if (player?.typeId !== "minecraft:player") return;
    let dimOk = false;
