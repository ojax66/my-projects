 ...
 *
 * A profundidade filtra a lista: silício raso, diamante fundo.
 *
 * Devolve o TIPO do minério ("silicon", "iron"...). Qual bloco é depende
 * também da pedra que está sendo substituída — cada minério existe na pedra do
 * meio e na ardósia funda, como no jogo base.
 */
...
/**
 * O bloco de minério: o tipo mais a pedra em que ele está.
 *
 * `space_dim:moon_silicon_ore` na pedra de regolito, `..._ore_deep` na ardósia.
 * Os dois blocos existem sempre — tools/assets/ores.json gera o par — então
 * nunca dá pra cair num id que não existe.
 */
function oreBlock(planet, ore, base) {
  const deep = base === planet.blocks.deep ? "_deep" : "";
  return `space_dim:${planet.id}_${ore}_ore${deep}`;
}
...
export function columnRunsAt(planet, x, z) {
  ...
  for (let i = 1; i <= planet.crust; i++) {
    ...
    if (base === planet.blocks.stone || base === planet.blocks.deep) {
      const ore = oreAt(planet, x, y, z, prof);
