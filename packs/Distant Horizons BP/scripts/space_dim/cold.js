...
export function applyCold(player) {
  ...
  if (r <= 0) {
    // Dano uma vez por segundo. Os frames de invulnerabilidade do jogo já
    // espaçam o dano real, mas aqui o intervalo é explícito pra o número em
    // COLD_DAMAGE querer dizer "por segundo" de verdade.
    if (system.currentTick % mc.TicksPerSecond === 0) {
      try { player.applyDamage(COLD_DAMAGE); } catch { }
