	...
	if fly:
		...
		if followed is Frog:
			...
			_status.text += "\nenergia %d%%  pele (hidratacao) %d%%  saude %d%%  idade %.1f dias  %s\ncerebro: %s (%d neuronios)   presa E/D %.0f/%.0f  ameaca E/D %.0f/%.0f\nmusculos extensores E/D %.2f/%.2f  pernas E/D %.2f/%.2f  orientar E/D %.2f/%.2f  LINGUA %.2f  fuga %.2f/%.2f  canto %.2f\njuntas E: quadril %.2f joelho %.2f tornozelo %.2f  ombro %.2f cotovelo %.2f  dedos abertos %.2f\nreflexos: boca %.2f  engolir %.2f  limpar %.2f/%.2f  abraco %.2f  grito %.2f  inflar %.2f\nhormonios: corticosterona %.2f  vasotocina %.2f  GnRH %.2f  melatonina %.2f  MSH %.2f  sede %.2f\norgaos: %s" % [
				...
				fr.limbs.leg["L"]["quadril"], fr.limbs.leg["L"]["joelho"], fr.limbs.leg["L"]["tornozelo"], fr.limbs.arm["L"]["ombro"], fr.limbs.arm["L"]["cotovelo"], fr.limbs.spread["L"],
